...
function pageHero(pageKey, eyebrow, title, copy) { return `<section class="page-hero"><div class="page-hero__inner"><p class="eyebrow">${eyebrow}</p><h1>${title}</h1><p class="hero-lead">${copy}</p></div></section>`; }
...
function processSection() { return `<section class="process-section"><div class="section-header"><p class="eyebrow">Process</p><h2>How we move.</h2><p class="section-subtitle">The work starts with a careful review, not assumptions. We look at the site, workflow, or customer path, discuss the improvements, then build the solution that best supports the business.</p></div><div class="process-grid">${processCards()}</div></section>`; }
